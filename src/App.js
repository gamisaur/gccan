  ...
  const handleClick = async (question) => {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        const answer = querySnapshot.docs[0].data().answer;
        setMessages(prev => [...prev, { type: "bot", text: answer }]);
        speakText(answer);
      } else {
        const noAnswer = "No response found.";
        setMessages(prev => [...prev, { type: "bot", text: noAnswer }]);
        speakText(noAnswer);
      }
    } catch (err) {
        ...
        <div className="flex flex-col items-center px-4 py-6 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row gap-8 sm:mt-40 mt-32 px-5">
            <div className="p-10 bg-white/10 backdrop-blur-md rounded-lg shadow-lg text-center w-full max-w-md text-white">
              <img src="/CCS logo.png" alt="GCCAN Logo" className="w-24 h-24 mx-auto mb-4" />
          ...
          <div className="fixed bottom-0 left-0 w-full bg-gray-100/20 backdrop-blur-md py-1 text-center text-sm text-white">
            <p>✉️ Email us at <span className="font-semibold">info@gordoncollege.edu.ph</span></p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="w-full sm:w-1/3 p-4">
        <div className="p-6 bg-white rounded-lg shadow-lg shadow-gray-900/50">
          ...
          <div className="flex justify-between items-center mb-4">
            <p className="text-gray-600">TTS: </p>
            <label className="flex items-center space-x-2">
              ...
              />
              <span className="text-sm text-gray-700">{ttsEnabled ? "On" : "Off"}</span>
            </label>
          </div>
          ...
          {selectedCategory && (
            <div className="mt-4">
              ...
              <p className="text-sm text-gray-600">Click a question to get an answer.</p>
              {/* Ideally, fetch questions dynamically — here are sample ones: */}
              {[
                "How do I enroll?",
